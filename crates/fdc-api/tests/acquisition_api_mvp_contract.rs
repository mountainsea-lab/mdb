...
use fdc_server::{
    run_barter_fixture_mvp_once, run_realtime_barter_envelope_stream, BoundedMarketDataMvpRunner,
    FdcServerApp, RealtimeMarketDataMvpConfig,
};
...
#[tokio::test]
async fn realtime_mvp_stream_populates_store_read_by_api_route() {
    let store = Arc::new(QueryableMarketDataStore::new());
    let live_style_stream = stream::iter(vec![
        sample_envelope("BTCUSDT", "btc-live-1", "seq-live-1"),
        sample_envelope("ETHUSDT", "eth-live-1", "seq-live-2"),
        sample_envelope("BTCUSDT", "btc-live-2", "seq-live-3"),
    ]);

    let result = run_realtime_barter_envelope_stream(
        live_style_stream,
        Arc::clone(&store),
        RealtimeMarketDataMvpConfig {
            runtime_window: std::time::Duration::from_secs(1),
            idle_timeout: std::time::Duration::from_millis(50),
            max_errors: 0,
        },
    )
    .await
    .expect("realtime MVP runner should write live-style stream data");
...
async fn ignored_live_smoke_writes_binance_trade_to_store_and_reads_it_through_api() {
    ...
    let result = run_realtime_barter_envelope_stream(
        ...
        Arc::clone(&store),
        RealtimeMarketDataMvpConfig {
            runtime_window: std::time::Duration::from_secs(5),
            idle_timeout: std::time::Duration::from_millis(100),
            max_errors: 0,
        },
    )
    .await
    .expect("live envelopes should write through the realtime MVP runner");
